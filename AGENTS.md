...
- `app/(tabs)/settings.tsx` was split into sections under `src/components/settings/`, but the screen
  still owns roughly 1,000 lines of probe and action orchestration. Continue moving domain-specific
  state/actions into the owning sections or hooks instead of growing the screen. Local state
  (password inputs, model title/url/checksum, map search/browse) belongs to its section; download
  recovery detail UI lives in `DownloadsCard`.
