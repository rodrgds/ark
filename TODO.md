...
- [ ] ~ **Upgrade RAG from FTS-only to vector search** — Partially implemented. Installed content packs, notes, imported text documents, imported image OCR text, and best-effort native ZIM search results now feed local AI citations. RAG chunks use the upgraded `ark-hash-v2` 256-dimension deterministic offline embedding with token/phrase/character features and domain aliases for reranking, plus a versioned sqlite-vec table when the extension is available. Imported images run Android on-device ML Kit OCR in dev builds and are indexed into RAG without sending data off-device. Still needed: on-device sqlite-vec verification, model-quality embedding GGUF support through llama.rn embedding contexts, full PDF text extraction, and large-Wikipedia paragraph extraction beyond search-result/article snippets.
...
- [x] **Index imported documents and image OCR for RAG** — Imported text files are extracted into `documents.extracted_text`, imported images run Android on-device OCR through the `ark-ocr` Expo module, Library/document detail show OCR/indexing state, and document deletion removes the matching RAG source. Expo Go/dev builds without the OCR module report OCR as unavailable instead of fabricating text.
