...
### Boot / Stores
- [x] **`src/stores/auth-store.ts`** — `lock()` is fire-and-forget. *Fix: return Promise, `await`.* **VERIFIED — already returns `Promise<void>` and call sites in `_layout.tsx`/`AppShell` `await` it. No change needed.** *(Boot idempotency moved up to the Boot/Onboarding section to dedupe.)*
