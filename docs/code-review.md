...
```tsx
const networkStatusClass = isOnline === null ? 'bg-muted-foreground' : isOnline ? 'bg-green-500' : 'bg-red-500';
```
...
*Generated by comprehensive multi-agent code review. Each finding verified against actual source code.*
